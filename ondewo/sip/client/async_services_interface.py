...
"""Async services base that injects the Keycloak bearer token into every SIP gRPC call.

The generated async service wrappers (``ondewo/sip/client/services/async_*.py``) subclass
...
class AsyncServicesInterface(AsyncBaseServicesInterface, ABC):
    ...
    :class:`~ondewo.sip.utils.keycloak.KeycloakTokenProvider` that is shared per
    ...
    def metadata(self) -> List[Tuple[str, str]]:
        ...
        unauthenticated. ``bearer_metadata()`` is a synchronous thread-based read, safe to
