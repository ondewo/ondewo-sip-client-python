...
"""Unit tests for the SIP services-interface Keycloak-bearer injection seam.
...
def config_without_keycloak() -> ClientConfig:
    """A config with credentials but no Keycloak fields (``use_keycloak`` is False).

    Returns:
        ClientConfig: A config pointing to ``localhost:50051`` with no Keycloak auth.
    """
...
def config_with_keycloak() -> ClientConfig:
    """A config carrying the complete Keycloak ROPC field set (``use_keycloak`` is True).
